...
export const generateTikTokScript = async (
  ...
  const prompt = `Tạo kịch bản TikTok viral cho "${productName}". USP: "${keyword}". ${sizeInfo} ${noteInstruction} Xưng "em", gọi "anh chị". ${voiceDetail}. ${forbiddenWords} Dưới 190 ký tự mỗi phần.`;
...
export const generateCarouselScript = async (
  ...
  const prompt = `
    ...
    Ghi chú thêm: ${notes}.
    ${strategyNote}
    YÊU CẦU: Mỗi slide là 1 câu ngắn gọn, súc tích, gây ấn tượng mạnh, phù hợp để chèn lên ảnh.
