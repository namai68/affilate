...
export const suggestDhbcPhrases = async (): Promise<DhbcPhrase[]> => {
  const ai = getAiClient();
  const prompt = `Gợi ý 10 câu ngẫu nhiên để chơi trò "Đuổi hình bắt chữ". 
  Nội dung bao gồm: hot trends hiện nay, ca dao tục ngữ Việt Nam, thành ngữ, câu slang của giới trẻ (Gen Z).
  ...
  Ví dụ: "Chó cắn áo rách" có thể thành "_h_ ___ __ _á__".
...
export const generateDhbcIllustration = async (
  ...
  try {
    ...
    console.error("Dhbc Image Generation Error:", error);
    throw error;
  }
};
