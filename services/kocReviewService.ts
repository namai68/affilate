...
const getVoiceDetailedInstruction = (voiceLabel: string) => {
  const mapping: Record<string, string> = {
    "Giọng Bắc 20-40 tuổi": "20-40 tuổi, giọng miền Bắc, năng động, nhịp độ nhanh, vui vẻ, tông cao, hào hứng. Dùng từ: nhé, ạ, thế, chứ. Không dùng: nha, nè, thiệt, hông.",
    "Giọng Nam 20-40 tuổi": "20-40 tuổi, giọng miền Nam, năng động, nhịp độ nhanh, vui vẻ, tông cao, hào hứng. Dùng từ: nha, nè, thiệt, hông. Không dùng: nhé, ạ, thế.",
    ...
    "Giọng Bắc 60-80 tuổi": "60-80 tuổi, giọng miền Bắc, khàn, hào sảng, chân chất, thực tế, hào hứng. Dùng từ: nhé, ạ, thế, chứ.",
    "Giọng Nam 60-80 tuổi": "60-80 tuổi, giọng miền Nam, khàn, hào sảng, chân chất, thực tế, hào hứng. Dùng từ: nha, nè, thiệt, hông."
  };
...
export const generateKocScript = async (
  ...
  const prompt = `
    ...
    Mỗi phần kịch bản (v1, v2...) BẮT BUỘC PHẢI CÓ ĐỘ DÀI TỪ 160 ĐẾN 190 KÝ TỰ. 
    - Tuyệt đối KHÔNG ĐƯỢC NGẮN HƠN 160 ký tự.
    - Tuyệt đối KHÔNG ĐƯỢC DÀI HƠN 190 ký tự.
...
export const regenerateKocScriptPart = async (
  ...
  const prompt = `
    ...
    2. Độ dài bắt buộc: từ 160 đến 190 ký tự.
...
export const generateKocImage = async (
  ...
  // !!! QUY TẮC ĐỘC QUYỀN KHUÔN MẶT NGHIÊM NGẶT !!!
  const identityRule = faceImagePart 
    ...
    : `DANH TÍNH: Người Việt Nam trưởng thành ${gender} khớp với mô tả: "${characterDescription}".`;

  // !!! QUY TẮC NHẤT QUÁN TRANG PHỤC TUYỆT ĐỐI !!!
  const outfitRule = outfitImagePart ? `
  ...
  const visualRules = `
    ...
    2. KHÔNG có các yếu tố giao diện người dùng (UI), KHÔNG bong bóng thoại, KHÔNG dấu mờ (watermark), KHÔNG phụ đề, KHÔNG EMOJI.
    3. Hình ảnh phải trông giống như một ẢNH CHỤP RAW chuyên nghiệp hoặc một bản KẾT XUẤT chuyên nghiệp.
  ...
  const backgroundRule = backgroundReferencePart 
    ...
    : `MÔI TRƯỜNG: Đặt cảnh trong một ${backgroundNote || 'bối cảnh phong cách sống hàng ngày'} tự nhiên. Phối cảnh của môi trường phải khớp với góc máy ảnh: "${cameraAngle || 'chính diện'}".`;
...
!!! CRITICAL RULE: ONE CONTINUOUS SHOT (QUAN TRỌNG: MỘT CÚ MÁY LIỀN MẠCH) !!!
  ...
  PHẦN 5: LỜI THOẠI (DÙNG CHO ĐỒNG BỘ): ✨ Người mẫu nói với đặc điểm ${voiceDetail} (Giới tính: ${voiceGender}): "${scriptText}". Khớp môi 1 cách hoàn hảo.
