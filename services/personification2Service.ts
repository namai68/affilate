...
export const splitScriptIntoSegments = async (originalScript: string, voice: string, addressing: string, segmentCount: number): Promise<string[]> => {
  ...
  const prompt = `
    ...
    2. Mỗi đoạn BẮT BUỘC có độ dài từ 140 đến 160 ký tự.
    3. Tuyệt đối không được ngắn hơn 140 ký tự và không dài quá 160 ký tự.
...
export const generatePersonifiedImage = async (
  ...
  const objectPresenceRule = isPersonified 
    ? `THIẾT KẾ VẬT THỂ: Một nhân vật 3D được nhân hóa dựa trên "${characterIdea || 'vật thể'}". Có đôi mắt to, biểu cảm sinh động. Có chân tay nhưng ngắn ngủn bé xíu dễ thương.`
    : `!!! QUY TẮC XÓA VẬT THỂ !!!: TUYỆT ĐỐI KHÔNG CÓ VẬT THỂ TRONG ẢNH.`;
  ...
  const personIdentityRule = personParts.length > 0
    ...
    : `NHÂN VẬT NGƯỜI: Người Việt Nam (${gender}) đang trong cảnh quay.`;
