...
export const generateNonFaceScript = async (
  ...
  const prompt = `
    ...
    ĐỘ DÀI: 160-190 ký tự mỗi phần. XƯNG HÔ: "${addressing}".
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: { responseMimeType: "application/json", responseSchema: { type: Type.OBJECT, properties, required } }
    });
...
export const regenerateNonFaceScriptPart = async (
  ...
  const ai = getAiClient();
  const voiceDetail = getVoiceDetailedInstruction(voice);
  const prompt = `Viết lại lời thoại TikTok cho giày "${productName}". Nội dung cũ: "${currentPartContent}". Tệp khách hàng: "${targetAudience}". Xưng hô: "${addressing}". Giọng: ${voiceDetail}. Độ dài: 160-190 ký tự. Chỉ trả về text mới.`;
  try {
    const response = await ai.models.generateContent({ model: "gemini-3-flash-preview", contents: { parts: [{ text: prompt }, ...imageParts.map(p => ({ inlineData: p }))] } });
...
export const generateFootwearVeoPrompt = async (
  ...
  PHẦN 2: CHUYỂN ĐỘNG CAMERA (9:16). Dolly in, Pan hoặc Zoom mượt mà vào chi tiết sản phẩm một cách chuyên nghiệp hoặc tracking theo bước đi. Tuyệt đối không thay đổi bối cảnh và sản phẩm suốt 8s. 
  ...
  PHẦN 4: CHẤT LƯỢNG KỸ THUẬT: 4K, 60fps, Cinematic Studio Lighting. 
