      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-orange-100">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <div className="flex-1">
                    ...
                    <input 
                      ...
                      className="w-full p-3 bg-orange-50/30 border border-orange-100 rounded-xl focus:ring-2 focus:ring-orange-200 focus:border-orange-500 outline-none font-bold text-xs"
                    />
                  </div>
                  <div className="w-20">
                    <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">SIZE:</label>
                    <input 
                      type="number" 
                      value={state.titleFontSize}
                      onChange={e => setState(p => ({ ...p, titleFontSize: parseInt(e.target.value) || 10 }))}
                      className="w-full p-3 bg-orange-50/30 border border-orange-100 rounded-xl focus:ring-2 focus:ring-orange-200 outline-none font-bold text-xs"
                    />
                  </div>
                </div>

                <div>
                  <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">ĐÁP ÁN (Answer):</label>
                  <input 
                    type="text" 
                    value={state.answer}
                    onChange={e => {
                ...
                <div className="flex gap-2">
                  <div className="flex-1">
                    <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">CÂU ĐỐ (Scrambled Puzzle):</label>
                    <input 
                      type="text" 
                      value={state.puzzle}
              ...
              <div className="pt-2">
                <p className="text-[10px] font-black text-orange-400 uppercase mb-2">Gợi ý câu đố nhanh:</p>
                <div className="flex flex-wrap gap-2">
                  {SUGGESTED_PUZZLES.map((s, i) => (
                    <button 
                      key={i}
                      onClick={() => handleSelectSuggestion(s.p, s.a)}
                      className="px-3 py-1.5 bg-orange-50 hover:bg-orange-100 hover:text-orange-600 rounded-lg text-[10px] font-bold transition-all border border-orange-100 hover:border-orange-200"
                    >
                      {s.a}
                    </button>
                  ))}
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-orange-100">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">Ảnh nhân vật:</label>
                <div 
                  onClick={() => faceInputRef.current?.click()}
                  className="w-full h-24 border-2 border-dashed border-orange-100 rounded-xl flex items-center justify-center bg-orange-50/30 cursor-pointer hover:border-orange-400 transition-all overflow-hidden"
                >
                  {state.facePreview ? (
                    <img src={state.facePreview} className="h-full object-cover" alt="face" />
                  ) : (
                    <div className="text-center">
                      <svg className="w-5 h-5 text-orange-200 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                      <span className="text-[9px] font-bold text-orange-300 uppercase tracking-widest">Tải ảnh mặt</span>
                    </div>
                  )}
                ...
                <input 
                  ...
                  className="w-full mt-2 p-3 bg-orange-50/30 border border-orange-100 rounded-xl text-xs focus:ring-1 focus:ring-orange-500 outline-none"
                />
              </div>
              
              <div>
                <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">Màu nền tiêu đề:</label>
                <input 
                  type="color" 
                  value={state.headerColor}
                  onChange={e => setState(p => ({ ...p, headerColor: e.target.value }))}
                  className="w-full h-10 p-1 bg-orange-50/30 border border-orange-100 rounded-xl cursor-pointer"
                />
              </div>
              <div>
                ...
                <textarea 
                  ...
                  className="w-full p-3 bg-orange-50/30 border border-orange-100 rounded-xl text-xs focus:ring-1 focus:ring-orange-500 outline-none h-20 resize-none font-medium"
                />
              </div>
            </div>
            <button 
              ...
              className="w-full mt-6 py-4 bg-gradient-to-r from-orange-600 to-red-600 hover:shadow-orange-500/20 text-white font-black rounded-2xl shadow-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2 uppercase tracking-widest text-sm"
            >
        ...
        <div className="lg:col-span-8 flex flex-col items-center">
          <div className="relative aspect-[9/16] bg-orange-900 rounded-[32px] overflow-hidden shadow-2xl flex flex-col w-full max-w-sm border-8 border-orange-800">
            <div 
            ...
            <div className="h-[80%] relative bg-orange-50 overflow-hidden">
              {state.generatedImageUrl ? (
                <img src={state.generatedImageUrl} className="w-full h-full object-cover" alt="metaphor" />
              ) : (
                <div className="absolute inset-0 flex flex-col items-center justify-center p-10 text-center text-orange-200">
          ...
          <div className="mt-8 flex gap-4 w-full max-w-sm">
            <button 
              ...
              className="flex-1 py-4 bg-orange-500 hover:bg-orange-600 text-white font-black rounded-2xl flex items-center justify-center gap-2 transition-all disabled:opacity-30 uppercase text-xs tracking-widest shadow-xl"
            >
              Tải ảnh
            </button>
            <button 
              ...
              className="flex-1 py-4 bg-gradient-to-r from-orange-600 to-red-600 hover:shadow-orange-500/20 text-white font-black rounded-2xl flex items-center justify-center gap-2 transition-all disabled:opacity-30 uppercase text-xs tracking-widest shadow-xl"
            >
              Prompt
            </button>
          </div>
          ...
          {state.isVideoPromptVisible && (
            <div className="mt-8 w-full max-w-2xl bg-orange-50 rounded-3xl p-6 border border-orange-100 animate-slideUp">
              <div className="flex justify-between items-center mb-4">
                <span className="text-[10px] font-black text-orange-600 uppercase tracking-widest">Kịch bản Video VEO-3</span>
                <button 
