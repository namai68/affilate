...
const DhbcModule: React.FC = () => {
  ...
  const handleSuggest = async () => {
    ...
    const suggestions = await service.suggestDhbcPhrases();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-orange-100">
            <h3 className="text-sm font-black text-orange-900 uppercase mb-4 tracking-wider flex items-center gap-2">
            ...
            <div className="space-y-4">
              <button 
                ...
                className="w-full py-2.5 bg-orange-50 text-orange-700 font-bold rounded-xl hover:bg-orange-100 transition-all text-xs flex items-center justify-center gap-2 uppercase tracking-widest"
              >
                {state.isSuggesting ? '...' : '✨ Gợi ý câu Hot Trends'}
              </button>

              {state.suggestedPhrases.length > 0 && (
                <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-2 bg-orange-50/30 rounded-xl border border-orange-50 custom-scrollbar">
                  {state.suggestedPhrases.map((s, idx) => (
                    <button 
                      key={idx}
                      onClick={() => handleSelectSuggestion(s)}
                      className="px-3 py-1.5 bg-white border border-orange-100 hover:border-orange-400 hover:text-orange-600 rounded-lg text-[10px] font-bold transition-all shadow-sm"
                    >
                      {s.phrase}
                    </button>
                  ))}
              ...
              <div>
                <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">CÂU ĐỐ:</label>
                <textarea 
                  value={state.phrase}
                  onChange={e => {
                  ...
                  className="w-full p-3 bg-orange-50/30 border border-orange-100 rounded-xl focus:ring-2 focus:ring-orange-100 focus:border-orange-500 outline-none font-bold text-sm h-24 resize-none leading-[1.2]"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-orange-100">
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-black text-orange-400 uppercase block px-1 mb-1">Phong cách ảnh</label>
              ...
              <div>
                <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">Tiêu đề Header:</label>
                <div className="flex gap-2">
                  <input 
                    type="text" 
                    value={state.headerTitle}
                    onChange={e => setState(p => ({ ...p, headerTitle: e.target.value }))}
                    className="flex-1 p-3 bg-orange-50/30 border border-orange-100 rounded-xl text-xs font-bold outline-none focus:border-orange-500 leading-[1.2]"
                  />
                  <input 
                    type="number" 
                    value={state.headerFontSize}
                    onChange={e => setState(p => ({ ...p, headerFontSize: parseInt(e.target.value) || 10 }))}
                    className="w-16 p-3 bg-orange-50/30 border border-orange-100 rounded-xl text-xs font-bold outline-none focus:border-orange-500"
                    min="10"
                    max="100"
                  />
                </div>
              ...
              <div>
                <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">Ảnh mặt mẫu (Tùy chọn):</label>
                <div 
                  onClick={() => faceInputRef.current?.click()}
                  className="w-full h-20 border-2 border-dashed border-orange-100 rounded-xl flex items-center justify-center bg-orange-50/30 cursor-pointer hover:border-orange-400 transition-all overflow-hidden"
                >
                  {state.facePreview ? (
                    <img src={state.facePreview} className="h-full object-cover" alt="face" />
                  ) : (
                    <div className="text-center">
                      <svg className="w-5 h-5 text-orange-300 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                      <span className="text-[9px] font-bold text-orange-400 uppercase tracking-widest">Tải ảnh</span>
                    </div>
                  )}
                ...
                <input 
                  ...
                  className="w-full mt-2 p-3 bg-orange-50/30 border border-orange-100 rounded-xl text-xs focus:ring-1 focus:ring-orange-500 outline-none leading-[1.2]"
                />
              </div>
              
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">Màu Header:</label>
                  <input 
                    type="color" 
                    value={state.headerColor}
                    onChange={e => setState(p => ({ ...p, headerColor: e.target.value }))}
                    className="w-full h-10 p-1 bg-white border border-orange-100 rounded-lg cursor-pointer"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">Màu Ô chữ:</label>
                  <input 
                    type="color" 
              ...
              <div>
                <label className="text-[10px] font-black text-orange-400 uppercase block mb-1">Ghi chú sửa ảnh:</label>
                <textarea 
                  value={state.regenNote}
                  onChange={e => setState(p => ({ ...p, regenNote: e.target.value }))}
                  placeholder="Ghi chú thêm..."
                  className="w-full p-3 bg-orange-50/30 border border-orange-100 rounded-xl text-xs focus:ring-1 focus:ring-orange-500 outline-none h-20 resize-none font-medium leading-[1.2]"
                />
              </div>
            </div>
          </div>
          ...
          <button 
            ...
            className="w-full py-4 bg-orange-600 hover:bg-orange-700 text-white font-black rounded-2xl shadow-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2 uppercase tracking-widest text-sm"
          >
            {state.isLoading ? "ĐANG TẠO ẢNH..." : "BẮT ĐẦU VẼ ĐHBC"}
          </button>
        </div>

        <div className="lg:col-span-8 flex flex-col items-center pt-[10px]">
          <div className="relative aspect-[9/16] bg-gradient-to-r from-orange-600 to-red-600 rounded-[32px] overflow-hidden shadow-2xl flex flex-col w-full max-w-sm border-8 border-orange-900">
            <div 
            ...
            <div className="h-[50%] relative bg-orange-50 overflow-hidden">
              {state.generatedImageUrl ? (
                <img src={state.generatedImageUrl} className="w-full h-full object-cover" alt="illustration" />
              ) : (
                <div className="absolute inset-0 flex flex-col items-center justify-center p-10 text-center text-orange-300">
                  <p className="text-[10px] font-black uppercase tracking-widest">Minh họa (5/10) sẽ ở đây</p>
                </div>
              )}
              
              {state.isLoading && (
                <div className="absolute inset-0 bg-orange-900/40 backdrop-blur-sm flex items-center justify-center z-20">
                  <div className="text-center">
            ...
            >
              {(state.hint || state.phrase).replace(/\s/g, '').split('').map((char, i) => (
                <div key={i} className="w-8 h-8 bg-white flex items-center justify-center shadow-sm border border-orange-100 rounded-sm">
                  <span className="text-sm font-black text-orange-900">{char !== '_' ? char.toUpperCase() : ''}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-8 flex gap-4 w-full max-w-sm">
            <button 
              ...
              className="flex-1 py-4 bg-orange-500 hover:bg-orange-600 text-white font-black rounded-2xl flex items-center justify-center gap-2 transition-all disabled:opacity-30 uppercase text-xs tracking-widest shadow-lg active:scale-95"
            >
              Tải ảnh
            </button>
            <button 
              ...
              className="flex-1 py-4 bg-gradient-to-r from-orange-600 to-red-600 text-white font-black rounded-2xl flex items-center justify-center gap-2 transition-all disabled:opacity-30 uppercase text-xs tracking-widest shadow-lg active:scale-95"
            >
              Prompt
            </button>
          </div>
          ...
          {state.isVideoPromptVisible && (
            <div className="mt-8 w-full max-w-2xl bg-white rounded-3xl p-6 border border-orange-100 shadow-xl animate-slideUp">
              <div className="flex justify-between items-center mb-4">
                <span className="text-[10px] font-black text-orange-600 uppercase tracking-widest">Prompt Video (Tiếng Việt)</span>
                <button 
                  onClick={() => navigator.clipboard.writeText(state.videoPrompt)}
                  className="text-[10px] font-black text-orange-500 hover:text-orange-700 underline transition-colors"
                >
                  COPY
                </button>
              </div>
